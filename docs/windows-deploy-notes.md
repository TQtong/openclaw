...
OpenClaw 支持 Docker 容器化部署，适合服务器环境或需要隔离的场景。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `OPENCLAW_IMAGE` | `openclaw:local` | Docker 镜像，设置为远程镜像可跳过本地构建 |
| `OPENCLAW_GATEWAY_PORT` | `18789` | Gateway 端口 |
| `OPENCLAW_GATEWAY_BIND` | `lan` | 绑定模式：`lan` 或 `loopback` |
